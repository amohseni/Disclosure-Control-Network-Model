...
ui <- dashboardPage(
  ...
  dashboardHeader(title = HTML("Selective Self-Disclosure & Perceived Polarization <br> in Social Networks"), titleWidth = 600), 
  ...
  dashboardSidebar(
    ...
    fluidRow(
      ...
      column(
        ...
        sliderInput("N", "Number of Agents (N):", min = 1, max = 100, value = 20),
        helpText("Number of agents in the network."),
        sliderInput("L", "Length of Type Vector (L):", min = 1, max = 10, value = 2),
        helpText("Number of traits per agent."),
        sliderInput("T", "Number of Rounds (T):", min = 1, max = 100, value = 20),
        helpText("Total number of simulation rounds."),
        sliderInput("delta", "Influence Decay Factor (δ):", min = 0.1, max = 1.0, value = 0.5, step = 0.05),
      ...
      column(
        ...
        selectInput(
          "network_type",
          "Network Type:",
          choices = c("Erdős-Rényi" = "ER", "Watts-Strogatz" = "WS", "Barabási-Albert" = "BA")
        ),
        helpText("Select the network generation algorithm."),
        
        conditionalPanel(
          condition = "input.network_type == 'ER'",
          sliderInput("p_er", "Edge Probability (p):", min = 0.01, max = 0.5, value = 0.1, step = 0.01)
        ),
        helpText("Probability of edge creation in an ER network."),
        
        conditionalPanel(
          condition = "input.network_type == 'WS'",
          sliderInput("k_ws", "Initial Neighbors (k):", min = 2, max = 10, value = 4, step = 1),
          helpText("Initial number of neighbors in a WS network."), 
          sliderInput("p_ws", "Rewiring Probability (p):", min = 0.01, max = 0.5, value = 0.1, step = 0.01),
          helpText("Probability of rewiring connections in a WS network.")
        ),
        
        conditionalPanel(
          condition = "input.network_type == 'BA'",
          sliderInput("m_ba", "Edges per New Node (m):", min = 1, max = 10, value = 3, step = 1),
          helpText("Number of edges to attach from each new node in a BA network."),
        ),
        
        selectInput("init_type", "Type Vector Initialization:", choices = c("Random" = "random", "Polarized" = "polarized")),
        helpText("Choose how agent traits are initialized."),
        
        conditionalPanel(
          condition = "input.init_type == 'polarized'",
          sliderInput("b", "Bias Parameter (b):", min = 0.5, max = 0.99, value = 0.7, step = 0.01),
          helpText("Controls the degree of polarization.")
        ),
        selectInput("model_version", "Network Version:", choices = c("Static" = "static", "Dynamic" = "dynamic")),
        helpText("Choose whether the network remains static or evolves dynamically."),
        
        selectInput("disclosure_type", "Disclosure Type:", choices = c("Selective" = "selective", "Global" = "global")),
        helpText("Determine if disclosures are made selectively or globally."),
        
        conditionalPanel(
          condition = "input.disclosure_type == 'selective'",
          uiOutput("s_ui"),  # This outputs the slider with the new label.
          helpText("Selective Disclosure Size: Number of agents that reveal their traits.")
        )
      )
    )
  ),
  ...
  create_plot <- function() {
    ...
    if (plot_type == "Similarity") {
      p <- ggplot(df, aes(x = round)) +
        geom_line(aes(y = neighbor_similarity, color = "Neighbor Similarity"),
                  size = 1) +
        geom_line(aes(y = all_similarity, color = "All Similarity"), size = 1) +
        geom_line(
          aes(y = similarity_gap, color = "Similarity Gap"),
          size = 1,
          linetype = "dashed"
        ) +
        labs(
