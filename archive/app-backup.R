...
source("Disclosure-Network-Simulation.R", local = TRUE)
        ...
        tabPanel("Comparisons", fluidRow(
          column(
            ...
            div(
              ...
              hr(),
              helpText(HTML("<strong>Tip:</strong> Select 1-3 metrics for more readable plots"))
            )
          ), 
          column(9, 
            # Unified container for plot - we'll handle showing/hiding in the server
...
server <- function(input, output, session) {
  ...
  output$trend_plotly <- renderPlotly({
    ...
    # Print what metrics we're using
    print("DEBUG: Normal metrics:")
    print(normal_metrics)
    print("DEBUG: Gap metrics:")
    print(gap_metrics)
    ...
    if (data_param %in% names(df)) {
      ...
      if ((data_param == "disclosure_pct" || data_param == "s") && length(df[[data_param]][df[[data_param]] %in% x_breaks]) == 0) {
    ...
    # For non-similarity metrics, use the original plot style
    if (input$trend_group != "similarity") {
      # Define custom colors
      metric_colors <- colorRampPalette(RColorBrewer::brewer.pal(min(9, max(3, length(unique(summary_df$metric_label)))), "Set2"))(length(unique(summary_df$metric_label)))
      ...
      if (nrow(summary_df) == 0) {
        ...
        p <- ggplot(summary_df, aes(x = x, y = mean, colour = metric_label, linetype = metric_label)) +
          ...
          facet_wrap(~ model_version, scales = "free_y", 
                     labeller = labeller(model_version = c("static" = "Static Network", "dynamic" = "Dynamic Network"))) +
      ...
      # Return the plotly object for non-similarity metrics
      return(ggplotly(p, tooltip = c("x", "metric_label", "mean")) %>%
        layout(
          font = list(family = "Roboto"),
          margin = list(t = 80),
          title = list(
    ...
    dynamic_regular_data <- summary_df %>% 
      filter(model_version == "dynamic" & metric %in% normal_metrics)
    ...
    if (nrow(static_regular_data) > 0) {
      ...
      for (m in unique(static_regular_data$metric_label)) {
        ...
        if (nrow(df) > 0) {
          ...
          if (showInLegend) added_to_legend[[m]] <- TRUE
    ...
    return(plotly::subplot(
      static_regular_plot, dynamic_regular_plot,
      static_gap_plot, dynamic_gap_plot,
      nrows = 2,
      shareX = TRUE,
      margin = 0.05,
      titleX = TRUE,
